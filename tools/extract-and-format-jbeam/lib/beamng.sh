...
# `unzip -l` prints a column layout, so taking the last whitespace-separated
# field truncates any name containing a space. `-Z1` prints bare paths instead.
beamng_list_jbeam_files() {
  local zip="$1"
  unzip -Z1 "$zip" | grep '\.jbeam$'
}
